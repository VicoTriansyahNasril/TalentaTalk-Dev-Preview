...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _onLoginEvent(LoginEvent event, Emitter<AuthState> emit) async {
    ...
    try {
      ...
      dev.log(
        'Login success: token=${loginResponse.token}, userId=${loginResponse.userId}',
      );
      ...
      if (loginResponse.token.isNotEmpty) {
        ...
        dev.log('Saved token: $savedToken');
        dev.log('Saved userId: $savedUserId');
        ...
        try {
          ...
          if (onboardingResponse.showOnboarding) {
            emit(AuthOnboardingRequired(token: loginResponse.token));
          } else {
            emit(AuthAuthenticated(token: loginResponse.token));
          }
        } catch (onboardingError) {
          dev.log('Onboarding check error: $onboardingError');
          emit(AuthAuthenticated(token: loginResponse.token));
        }
      } else {
    ...
    try {
      ...
      if (token != null && token.isNotEmpty) {
        try {
          ...
          dev.log('Onboarding check error on auth status: $onboardingError');

          if (_isTokenExpiredError(onboardingError.toString())) {
            dev.log(
              'Token expired, clearing credentials and redirecting to login',
            );
            await prefs.remove('token');
            await prefs.remove('userId');

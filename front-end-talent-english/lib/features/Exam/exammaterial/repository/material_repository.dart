...
class MaterialRepository {
  ...
  Future<List<MateriUjian>> fetchMaterials({String category = "default"}) async {
    return await service.fetchMaterialsByCategory(category);
  }
  
  Future<List<MateriUjian>> fetchMaterialsForPhonemes(List<String> phonemes) async {
    List<MateriUjian> allMaterials = [];
    
    List<String> categoryVariations = _generateCategoryVariations(phonemes);
    
    for (String category in categoryVariations) {
      try {
        print('Trying exam category: $category');
        final materials = await service.fetchMaterialsByCategory(category);
        
        if (materials.isNotEmpty) {
          allMaterials.addAll(materials);
          print('Found ${materials.length} exam materials for category: $category');
        } else {
          print('No exam materials found for category: $category');
        }
      } catch (e) {
        print('Error fetching exam materials for category $category: $e');
      }
    }
    
    if (allMaterials.isEmpty) {
      print('No exam materials found for any category variations of phonemes: ${phonemes.join(', ')}');
      return [];
    }
    
    final Map<int, MateriUjian> uniqueMaterials = {};

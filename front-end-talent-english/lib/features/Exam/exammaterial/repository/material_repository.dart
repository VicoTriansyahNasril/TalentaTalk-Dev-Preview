...
class MaterialRepository {
  ...
  Future<List<MateriUjian>> fetchMaterials({
    String category = "default",
  }) async {
    return await service.fetchMaterialsByCategory(category);
  }

  Future<List<MateriUjian>> fetchMaterialsForPhonemes(
    ...
    for (String category in categoryVariations) {
      try {
        dev.log('Trying exam category: $category');
        final materials = await service.fetchMaterialsByCategory(category);

        if (materials.isNotEmpty) {
          allMaterials.addAll(materials);
          dev.log(
            'Found ${materials.length} exam materials for category: $category',
          );
        } else {
          dev.log('No exam materials found for category: $category');
        }
      } catch (e) {
        dev.log('Error fetching exam materials for category $category: $e');
      }
    }

    if (allMaterials.isEmpty) {
      dev.log(
        'No exam materials found for any category variations of phonemes: ${phonemes.join(', ')}',
      );
      return [];
    }
  ...
  List<List<String>> _generatePermutations(List<String> list) {
    ...
    for (int i = 0; i < list.length; i++) {
      ...
      List<List<String>> remainingPermutations = _generatePermutations(
        remaining,
      );

...
class PronunciationSentencesBloc
    extends Bloc<PronunciationEvent, PronunciationSentencesState> {
  ...
  Future<void> _onStopRecording(
    ...
    dev.log('🕒 isSubmitting: ${state.isSubmitting}');
    if (path == null || state.currentSentence == null) return;

    try {
      dev.log(
        '🎙️ Sending recording for evaluation (with examId: ${state.examId})',
      );
      dev.log('🎙️ Sen kalimat: ${state.currentSentence!.id})');
      ...
      dev.log('📥 Response from /exam/compare: $body');
      ...
      if (response.statusCode == 200) {
        ...
        dev.log(
          '🔍 Raw similarity percent: $similarityPercentStr (type: ${similarityPercentStr.runtimeType})',
        );
      ...
      if (response.statusCode == 200) {
        ...
        for (var item in data) {
          try {
            ...
            dev.log('📥Processing exam item: $item');
            ...
            dev.log('❌ Error parsing exam item: $e, Item: $item');
          }
        }
        ...
        if (sentences.isEmpty) {
          emit(
            state.copyWith(
              errorMessage:
                  'Could not parse any valid sentences from the exam response',
              isLoading: false,
            ),
          );
          return;
        ...
        dev.log(
          '📥 Fetched ${sentences.length} sentences for exam ${event.examId}',
        );
      } else {
        emit(
          state.copyWith(
            errorMessage:
                'Failed to fetch exam sentences: Status ${response.statusCode}',
            isLoading: false,
          ),
        );
      }
    } catch (e) {
      dev.log('❌ Error fetching exam sentences: $e');
      emit(
        state.copyWith(
    ...
    try {
      ...
      if (deleteResponse.statusCode != 200) {
        ...
        emit(
          state.copyWith(
            errorMessage:
                'Failed to reset exam: ${errorData['detail'] ?? 'Unknown error'}',
            isRestarting: false,
          ),
        );
        return;
      ...
      dev.log('🔄 Exam details deleted successfully');
      ...
      if (startResponse.statusCode == 200) {
        ...
        dev.log("🔄 Restart response: $responseData");
        ...
        for (var item in data) {
          try {
            ...
            dev.log('❌ Error parsing restart exam item: $e, Item: $item');
          }
        }
        ...
        if (sentences.isEmpty) {
          emit(
            state.copyWith(
              errorMessage:
                  'Could not parse any valid sentences from restarted exam',
              isRestarting: false,
            ),
          );
          return;
        ...
        dev.log(
          '🔄 Exam restarted successfully with ${sentences.length} sentences',
        );
      } else {
        emit(
          state.copyWith(
            errorMessage:
                'Failed to restart exam: Status ${startResponse.statusCode}',
            isRestarting: false,
          ),
        );
      }
    } catch (e) {
      dev.log('❌ Error restarting exam: $e');
      emit(
        state.copyWith(

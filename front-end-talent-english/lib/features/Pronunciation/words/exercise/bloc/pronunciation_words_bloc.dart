...
class PronunciationWordsBloc
    extends Bloc<PronunciationEvent, PronunciationWordsState> {
  ...
  Future<void> _onStopRecording(
    ...
    try {
      ...
      dev.log('🎙️ [SEND] Preparing to send request to /compare_word');
      dev.log('🆔 idContent: ${state.idContent}');
      dev.log('🧑 userId: $userId');
      dev.log('🔤 filter_phonemes: ${state.phonemeCategory}');
      dev.log('📁 audioPath: $path');
      dev.log('📏 fileSize: $fileLength bytes');
      ...
      if (response.statusCode == 200) {
        ...
        dev.log('🔄 New state emitted with score: $similarity');
      } else {
        emit(
          state.copyWith(
            errorMessage: '❌ Gagal dari server: ${response.statusCode}',
          ),
        );
      }
    } catch (e) {
  ...
  Future<void> _onFetchWordByPhoneme(
    ...
    try {
      dev.log('📢 Fonem yang diminta: ${event.phoneme}');
      final response = await http.get(
        Uri.parse('$baseUrl/phoneme/random_word/${event.phoneme}'),
      );

      if (response.statusCode == 200) {
        final data = json.decode(utf8.decode(response.bodyBytes));
        dev.log('📥 Data dari API: $data');
        if (data['content'] != null &&
          ...
          emit(
            state.copyWith(errorMessage: '⚠️ Tidak ada materi untuk fonem ini'),
          );
        }
      } else {
        emit(
  ...
  Future<void> _onFetchWordById(
    ...
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/phoneme/word_by_id/${event.id}'),
      );

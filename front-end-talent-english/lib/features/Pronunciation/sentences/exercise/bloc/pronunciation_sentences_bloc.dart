...
class PronunciationSentencesBloc
    ...
    if (await _recorder.hasPermission()) {
      if (_recordedPath != null) {
        ...
        if (await oldFile.exists()) {
          ...
          dev.log('🗑️ Rekaman lama dihapus: $_recordedPath');
        }
      }
  ...
  Future<void> _onStopRecording(
    ...
    try {
      dev.log(
        'Stop mulai dan Mengirim rekaman ke /hasil_controller dengan idContent: ${state.idContent}',
      );
      ...
      if (userId == null) {
        emit(
          state.copyWith(
            errorMessage: 'User ID tidak ditemukan di SharedPreferences',
          ),
        );
        return;
      }
      ...
      dev.log('📥 Response body: $body');

      if (response.statusCode == 200) {
        final data = json.decode(body);

        final similarityPercentStr = data['similarity_percent'] as String;
        final similarityPercent = double.parse(
          similarityPercentStr.replaceAll('%', ''),
        );
        ...
        final List<PhonemeResult> phonemeComparison =
            (data['phoneme_comparison'] as List)
                .map(
                  (item) =>
                      PhonemeResult.fromJson(item as Map<String, dynamic>),
                )
                .toList();
        ...
        dev.log(
          '📤 Menampilkan hasil /compare dengan idContent: ${state.idContent}',
        );
      }
    } catch (e) {
      dev.log('❌ Gagal kirim ke /compare: $e');
  ...
  void _onClearResults(
    ...
    emit(
      state.copyWith(phonemeResults: const [], phonemeResult: '', score: 0.0),
    );
  }
  ...
  Future<void> _onFetchSentenceByPhoneme(
    ...
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/phoneme/random_sentence/${event.phoneme}'),
      );
      if (response.statusCode == 200) {
        ...
        if (data['content'] != null && data['phoneme'] != null) {
          ...
          dev.log("⚠️ Tidak ada data untuk fonem tersebut");
        }
      } else {
        dev.log('❌ Server error: ${response.statusCode}');
      }
    } catch (e) {
      dev.log('❌ Error mengambil kalimat berdasarkan fonem: $e');
    }
  }
  ...
  Future<void> _onFetchSentenceById(
    ...
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/phoneme/sentence_by_id/${event.id}'),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data['content'] != null &&
          ...
          emit(
            state.copyWith(errorMessage: '⚠️ No word found for the given ID.'),
          );
        }
      } else {
        emit(

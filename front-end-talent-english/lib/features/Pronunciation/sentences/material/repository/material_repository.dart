...
class MaterialRepository {
  ...
  Future<List<MaterialModel>> fetchMaterialsForPhonemes(
    ...
    for (String category in categoryVariations) {
      try {
        dev.log('Trying category: $category');
        final materials = await service.fetchMaterialsByCategory(category);

        if (materials.isNotEmpty) {
          allMaterials.addAll(materials);
          dev.log(
            'Found ${materials.length} materials for category: $category',
          );
        } else {
          dev.log('No materials found for category: $category');
        }
      } catch (e) {
        dev.log('Error fetching materials for category $category: $e');
      }
    }

    if (allMaterials.isEmpty) {
      dev.log(
        'No materials found for any category variations of phonemes: ${phonemes.join(', ')}',
      );
      return [];
    }

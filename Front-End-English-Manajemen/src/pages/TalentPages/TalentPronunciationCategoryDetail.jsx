...
const TalentPronunciationCategoryDetail = () => {
  ...
  const fetchData = useCallback(async () => {
    if (!id || !decodedKategori) return;
    setLoading(true);
    setError(null);
    try {
      const response = await talentService.getTalentDetailProgress(id, "phoneme-exercise", decodedKategori, {
        page: paginationModel.page + 1,
        limit: paginationModel.pageSize,
      });

      setTalentData(response.talentInfo);
      setExercises(response.phonemeDetail?.sentences || []);
      setTotalData(response.pagination?.totalRecords || 0);
    } catch (err) {
      setError(err.message || "Failed to load pronunciation details");
    } finally {
      setLoading(false);
    }
  }, [id, decodedKategori, paginationModel]);
  ...
  const columns = [
    { field: "sentence", headerName: "Sentence", flex: 3, renderCell: (params) => <CustomTypography variant="body2" sx={{ whiteSpace: "normal" }}>{params.row.sentence}</CustomTypography> },
  ...
  return (
    <Box p={4}>
      <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 2 }}>
        ...
        <CustomTypography color="text.primary">Pronunciation Exercise Detail</CustomTypography>
      </Breadcrumbs>
      ...
      <Paper elevation={1} sx={{ p: 3 }}>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          <CustomTypography variant="subtitle1" fontWeight={600}>Pronunciation Exercise Detail: {decodedKategori}</CustomTypography>
...
export default TalentPronunciationCategoryDetail;